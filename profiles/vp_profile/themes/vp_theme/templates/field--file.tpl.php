    ...
    if (substr($element['#field_name'], 0, 17) === 'field_logo_files_') {
      foreach ($variables['element']['#items'] as $delta => $item) {
        ...
        if (user_access('access zipcart downloads')) {
          echo theme('zipcart_download', array('text' => drupal_strtoupper(pathinfo($item['uri'], PATHINFO_EXTENSION)), 'path' => variable_get('file_public_path', conf_path() . '/files' . str_replace('public://', '/', $item['uri']))));
        }
        else {
          echo l(drupal_strtoupper(pathinfo($item['uri'], PATHINFO_EXTENSION)), file_create_url($item['uri']), array('attributes' => array('class' => 'zipcart')));
        }
        echo '</div>';
      }
    }
